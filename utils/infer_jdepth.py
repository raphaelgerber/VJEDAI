...
If --ckpt is omitted it defaults to
    $SCRATCH/checkpoints/jepa_depth_<variant>/best_<loss_mode>.pth
trying loss_mode 'nll' then 'si_mse'.
"""
...
def parse_args():
    ...
    p.add_argument("--ckpt", default=None, help="path to trained checkpoint (.pth)")
...
def resolve_ckpt(args):
    ...
    ckpt_dir = scratch / "checkpoints" / f"jepa_depth_{args.variant}"
    for tag in ("nll", "si_mse"):
        cand = ckpt_dir / f"best_{tag}.pth"
        if cand.exists():
            print(f"--ckpt not given; using {cand}")
            return cand
    sys.exit(
        f"No checkpoint given and none found under {ckpt_dir} "
        f"(looked for best_nll.pth, best_si_mse.pth)."
    )
